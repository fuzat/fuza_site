...
@section('link_url')
    <div class="m-portlet__head-text text-right">
        <a href="{{ route('admin.partners.index') }}" class="btn btn-accent">
...
@section('form_input')
    ...
    {!! Form::open(['method' => !empty($id) ? 'PUT' : 'POST', 'route' => !empty($id) ? ['admin.partners.update', 'id' => $id] : ['admin.partners.store'], 'files' => true, 'class' => 'm-form m-form--fit m-form--label-align-right', ]) !!}
    <div class="m-portlet__body">
        ...
        {{--<div class="form-group m-form__group">--}}
            {{--<label>{{ __('validation.attributes.' . $prefix . 'url') }}</label>--}}
            {{--{!! Form::text($prefix . 'url', old($prefix . 'url', !empty($id) ? $data->url : ''), ['class' => 'form-control m-input']) !!}--}}
        {{--</div>--}}
        <div class="form-group m-form__group">
