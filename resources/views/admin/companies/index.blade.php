...
@section('link_url')
    <div class="m-portlet__head-text text-right">
        <a href="{{ route('admin.companies.create') }}" class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air">
...
@section('search')
    @php($f = \App\Models\Company::_ATTR . '-')
    {!! Form::open(['method' => 'GET', 'route' => ['admin.companies.index'], 'class' => 'm-form m-form--fit m-form--label-align-right']) !!}
    ...
    <div class="col-xl-4 order-1 order-xl-2">
        ...
        <a href="{{ route('admin.companies.index') }}" class="btn btn-secondary m-btn m-btn--custom m-btn--icon m-btn--air">
...
@section('list')
    <table class="table m-table m-table--head-separator-primary" id="table-company">
        <thead>
        <tr>
            <th>#</th>
            ...
            <th>{{ __('validation.attributes.'. $f .'working-time') }}</th>
            <th>{{ __('validation.attributes.'. $f .'work-location') }}</th>
        ...
        @foreach($data as $i => $item)
            <tr>
                ...
                <td>{{ strip_tags(optional($item->datameta('', 'working-time'))->data_value) }}</td>
                <td>
                    @if($item->work_locations->isNotEmpty())
                        @foreach($item->work_locations as $j => $location)
                            <strong>{{ strip_tags(optional($location->datameta('', 'city'))->data_value) }}: </strong>
                            <span>{{ strip_tags(optional($location->datameta('', 'address'))->data_value) }}</span>
                            <br>
                        @endforeach
                    @endif
                </td>
                <td>
                    @foreach(\App\Models\Datameta::getExistLanguages(['type' => \App\Models\Datameta::TYPE_COMPANY, 'id' => $item->id, 'field' => 'name']) as $k => $language)
                        @php($lang = str_replace('name_', '', $language->data_field))
                        <a href="{{ route('admin.companies.edit', ['id' => $item->id, 'lang' => $lang]) }}">
                ...
                <td class="">
                    <a href="{{ route('admin.companies.edit', ['id' => $item->id]) }}" class="btn btn-accent m-btn m-btn--icon m-btn--air">
                        <i class="la la-pencil"></i>  {{ __('custom.button.update') }}
                    </a>
                    <button class="btn btn-danger m-btn m-btn--icon m-btn--air" id="btn-delete" data-route="{{ route('admin.companies.destroy', ['id' => $item->id]) }}">
...
@section("script")
    <script type="text/javascript">
        function changeStatus(id) {
            $.blockUI({message: ''});
            $.ajax({
                url: route('admin.ajax.companies.change-status'),
                type:'POST',
                data: {id: id},
        ...
        $('#table-company').on('click', '#btn-delete', function (event) {
