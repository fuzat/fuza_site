...
@section('link_url')
    <div class="m-portlet__head-text text-right">
        <a href="{{ route('admin.positions.index') }}" class="btn btn-accent">
...
@section('form_input')
    {!! Form::open(['method' => !empty($id) ? 'PUT' : 'POST', 'route' => !empty($id) ? ['admin.positions.update', 'id' => $id] : ['admin.positions.store'], 'class' => 'm-form m-form--fit m-form--label-align-right', ]) !!}
    ...
    <script type="text/javascript">
        $(function () {
            $('#locale').on('change', function () {
                $.blockUI({message: ''});
                window.location.replace(route('admin.positions.edit', {id: '{!! !empty($id) ? $id : 0 !!}', lang: $(this).val()}), true);
            });
        });
    </script>
@endsection
