...
@section('link_url')
    <div class="m-portlet__head-text text-right">
        <a href="{{ route('admin.locations.create') }}" class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air">
...
@section('search')
    @php($f = \App\Models\Location::_ATTR . '-')
    {!! Form::open(['method' => 'GET', 'route' => ['admin.locations.index'], 'class' => 'm-form m-form--fit m-form--label-align-right']) !!}
    ...
    <div class="col-xl-4 order-1 order-xl-2">
        ...
        <a href="{{ route('admin.locations.index') }}" class="btn btn-secondary m-btn m-btn--custom m-btn--icon m-btn--air">
...
@section('list')
    <table class="table m-table m-table--head-separator-primary" id="table-location">
        <thead>
        <tr>
            <th>#</th>
        ...
        @foreach($data as $i => $item)
            <tr>
                ...
                <td>
                    @foreach(\App\Models\Datameta::getExistLanguages(['type' => \App\Models\Datameta::TYPE_LOCATION, 'id' => $item->id, 'field' => 'name']) as $k => $language)
                        @php($lang = str_replace('name_', '', $language->data_field))
                        <a href="{{ route('admin.locations.edit', ['id' => $item->id, 'lang' => $lang]) }}">
                ...
                <td class="">
                    <a href="{{ route('admin.locations.edit', ['id' => $item->id]) }}" class="btn btn-accent m-btn m-btn--icon m-btn--air">
                        <i class="la la-pencil"></i>  {{ __('custom.button.update') }}
                    </a>
                    <button class="btn btn-danger m-btn m-btn--icon m-btn--air" id="btn-delete" data-route="{{ route('admin.locations.destroy', ['id' => $item->id]) }}">
...
@section("script")
    <script type="text/javascript">
        function changeStatus(id) {
            $.blockUI({message: ''});
            $.ajax({
                url: route('admin.ajax.locations.change-status'),
                type:'POST',
                data: {id: id},
        ...
        $('#table-location').on('click', '#btn-delete', function (event) {
