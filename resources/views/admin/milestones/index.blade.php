...
@section('link_url')
    <div class="m-portlet__head-text text-right">
        <a href="{{ route('admin.milestones.create') }}" class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air">
...
@section('search')
    @php($f = !empty($page_setting) ? $page_setting['attr'] . '-' : \App\Models\Milestone::_ATTR . '-')
    {!! Form::open(['method' => 'GET', 'route' => ['admin.milestones.index'], 'class' => 'm-form m-form--fit m-form--label-align-right']) !!}
    ...
    <div class="col-xl-4 order-1 order-xl-2">
        ...
        <a href="{{ route('admin.milestones.index') }}" class="btn btn-secondary m-btn m-btn--custom m-btn--icon m-btn--air">
...
@section('list')
    <table class="table m-table m-table--head-separator-primary" id="table-milestone">
        <thead>
        <tr>
            <th>#</th>
            <th>{{ __('validation.attributes.'. $f .'file') }}</th>
            <th>{{ __('validation.attributes.'. $f .'name') }}</th>
            <th>{{ __('validation.attributes.'. $f .'year') }}</th>
        ...
        @foreach($data as $i => $item)
            <tr>
                ...
                <td>{{ strip_tags($item->year) }}</td>
                <td>{!! Illuminate\Support\Str::limit(strip_tags(optional($item->datameta('', 'description'))->data_value), 50) !!}</td>
                <td>{!! Illuminate\Support\Str::limit(strip_tags(optional($item->datameta('', 'hover'))->data_value), 50) !!}</td>
                <td>
                    @foreach(\App\Models\Datameta::getExistLanguages(['type' => \App\Models\Datameta::TYPE_MILESTONE, 'id' => $item->id, 'field' => 'name']) as $k => $language)
                        @php($lang = str_replace('name_', '', $language->data_field))
                        <a href="{{ route('admin.milestones.edit', ['id' => $item->id, 'lang' => $lang]) }}">
                ...
                <td class="">
                    <a href="{{ route('admin.milestones.edit', ['id' => $item->id]) }}" class="btn btn-accent m-btn m-btn--icon m-btn--air">
                        <i class="la la-pencil"></i>  {{ __('custom.button.update') }}
                    </a>
                    <button class="btn btn-danger m-btn m-btn--icon m-btn--air" id="btn-delete" data-route="{{ route('admin.milestones.destroy', ['id' => $item->id]) }}">
...
@section("script")
    <script type="text/javascript">
        function changeStatus(id) {
            $.blockUI({message: ''});
            $.ajax({
                url: route('admin.ajax.milestones.change-status'),
                type:'POST',
                data: {id: id},
        ...
        $('#table-milestone').on('click', '#btn-delete', function (event) {
