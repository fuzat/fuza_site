...
@section('link_url')
    <div class="m-portlet__head-text text-right">
        <a href="{{ route('admin.job-levels.create') }}" class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air">
...
@section('search')
    {!! Form::open(['method' => 'GET', 'route' => 'admin.job-levels.index', 'class' => 'm-form m-form--fit m-form--label-align-right']) !!}
    ...
    <div class="col-xl-12">
        ...
        <a href="{{ route('admin.job-levels.index') }}" class="btn btn-secondary m-btn m-btn--custom m-btn--icon m-btn--air">
...
@section('list')
    <table class="table m-table m-table--head-separator-primary" id="table-job-level">
        <thead>
        <tr>
            <th>#</th>
        ...
        @foreach($data as $i => $item)
            <tr>
                ...
                <td>
                    @foreach(\App\Models\Datameta::getExistLanguages(['type' => \App\Models\Datameta::TYPE_JOB_LEVEL, 'id' => $item->id, 'field' => 'name']) as $k => $language)
                        @php($lang = str_replace('name_', '', $language->data_field))
                        <a href="{{ route('admin.job-levels.edit', ['id' => $item->id, 'lang' => $lang]) }}">
                ...
                <td>
                    <a href="{{ route('admin.job-levels.edit', ['id' => $item->id]) }}" class="text-warning" title="{{ __('custom.button.update') }}">
                        <i class="la la-pencil"></i>
                    </a>
                    <a href="javascript:void(0);" class="text-danger" id="btn-delete" data-route="{{ route('admin.job-levels.destroy', ['id' => $item->id]) }}">
...
@section("script")
    <script type="text/javascript">
        function changeStatus(id) {
            $.blockUI({message: ''});
            $.ajax({
                url: route('admin.ajax.job-levels.change-status'),
                type:'POST',
                data: {id: id},
        ...
        $('#table-job-level').on('click', '#btn-delete', function (event) {
