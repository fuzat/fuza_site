...
@section('link_url')
    <div class="m-portlet__head-text text-right">
        <a href="{{ route('admin.job-levels.index') }}" class="btn btn-accent">
...
@section('form_input')
    {!! Form::open(['method' => !empty($id) ? 'PUT' : 'POST', 'route' => !empty($id) ? ['admin.job-levels.update', 'id' => $id] : ['admin.job-levels.store'], 'files' => true, 'class' => 'm-form m-form--fit m-form--label-align-right']) !!}
    ...
    <script type="text/javascript">
        $(function () {
            $('#locale').change(function () {
                $.blockUI({message: ''});
                return window.location.replace(route('admin.job-levels.edit', {id: '{!! !empty($id) ? $id : 0 !!}', lang: $(this).val()}), true);
            });
        });
    </script>
@endsection
